...
import com.birtu.oj.allsearch.nai.service.neo.*;
import com.birtu.oj.allsearch.nai.tengxunAPI.TengxunAPI;
import com.birtu.oj.allsearch.nai.util.Mould;
...
public class MethodCall{
     ...
     * @return
     */
    public Resp methodCall(Words words, String msg){
        this.msg = msg;
...
//  >>>>>>>>>>>>>>>>>>>>>>>>>>>>> Region Start <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    @Mould(format = "LOC+")
    private Resp reg(Words<String,String> words) throws NoSuchAlgorithmException, InvalidKeyException, UnsupportedEncodingException {
        return null;
    }
...
//    }
    ...
    @Mould(format = "PID+same+")
    private Resp samePid(Words<String,String> words){
        return problemService.getProblemByPid(words.get("PID")); 
    }

    @Mould(format = "PID+same+dif+")
    private Resp samedifPid(Words<String,String> words){
        return problemService.getProblemsByPidAndDiff(words.get("PID"));
    }

    @Mould(format = "PID+same+alg+")
    private Resp samealgPid(Words<String,String> words){
        return problemService.getProblemsByPidAndAlgorithm(words.get("PID"));
    }

    @Mould(format = "TITLE+same+dif+")
    private Resp samedifTitle(Words<String,String> words){
        return titleService.getProblemsByTitleAndDiff(words.get("TITLE"));
    }

    @Mould(format = "TITLE+same+alg+")
    private Resp samealgTitle(Words<String,String> words){
        return titleService.getProblemsByTitleAndAlgorithm(words.get("TITLE"));
    }
