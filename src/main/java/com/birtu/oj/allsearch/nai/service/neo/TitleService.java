...
import com.birtu.oj.allsearch.nai.comom.TagsCommom;
import com.birtu.oj.allsearch.nai.entry.dto.ProblemAndTags;
import com.birtu.oj.allsearch.nai.entry.node.*;
import com.birtu.oj.allsearch.nai.repository.TitleCql;
...
//        problemAndTags.setProblem(titleCql.findByVague(title));
     ...
     */
    @Deprecated
    public Resp getProblemsByTitleAndDiff(String diff){
        List<Problem> problemsByTitleAndDiff = titleCql.getProblemsByTitleAndDiff(diff,DEFAULT_SIZE);
        if(problemsByTitleAndDiff == null || problemsByTitleAndDiff.size() < 1){
            return new Resp(diff+"下暂时没有题目",null);
        }
        Resp resp = this.getProblemAndTags(problemsByTitleAndDiff);
        return resp;
    }
     ...
     */
    @Deprecated
    public Resp getProblemsByTitleAndAlgorithm(String alg){
        List<Problem> problemsByTitleAndAlgorithm = titleCql.getProblemsByTitleAndAlgorithm(alg);
        if(problemsByTitleAndAlgorithm == null || problemsByTitleAndAlgorithm.size() < 1){
            return new Resp(alg+"下暂时没有题目",null);
        }
        Resp resp = this.getProblemAndTags(problemsByTitleAndAlgorithm);
        return resp;
    }
